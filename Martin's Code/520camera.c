...
width and height of the viewport and three other parameters. The camera looks 
down the center of the viewing volume. For perspective projection, fovy is the 
...
void camRender(camCamera *cam, GLint viewingLoc) {
	...
	if (cam->projectionType == camORTHOGRAPHIC) {
        mat44Orthographic(
            cam->projection[camPROJL], cam->projection[camPROJR],
            cam->projection[camPROJB], cam->projection[camPROJT],
            cam->projection[camPROJF], cam->projection[camPROJN], proj);
    } else {
        mat44Perspective(
            cam->projection[camPROJL], cam->projection[camPROJR],
            cam->projection[camPROJB], cam->projection[camPROJT],
