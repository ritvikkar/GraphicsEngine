...
void interpolate(double a[], double b[], double c[],
    ...
    double bMinusA[renATTRDIMBOUND], cMinusA[renATTRDIMBOUND];
    vecSubtract(ren->attrDim,b,a,bMinusA);//b-a but sized attrDim
    vecSubtract(ren->attrDim,c,a,cMinusA);//c-a but sized attrDim

    double scaleP[ren->attrDim],scaleQ[ren->attrDim],addPQ[ren->attrDim];
    vecScale(ren->attrDim,pq[0],bMinusA,scaleP);//p(b-a)
    vecScale(ren->attrDim,pq[1],cMinusA,scaleQ);//q(c-a)

    vecAdd(ren->attrDim,scaleP,scaleQ,addPQ);
    double khi[ren->attrDim];
    vecAdd(ren->attrDim,addPQ,a,khi);

    double rgb[3];
    colorPixel(ren, unif, tex, khi, rgb);
...
void triRender(renRenderer *ren, double unif[], texTexture *tex[], double a[], 
        double b[], double c[])
{
    formatTriangle(a,b,c,ren->attrDim);
