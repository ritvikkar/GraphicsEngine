...
void meshGLInitialize(meshGLMesh *meshGL, meshMesh *mesh) {
	...
	glGenBuffers(2, meshGL->buffers); //generate the 2 buffers
	glBindBuffer(GL_ARRAY_BUFFER, meshGL->buffers[0]); //bind them to a buffer 0
	glBufferData(GL_ARRAY_BUFFER, mesh->vertNum * mesh->attrDim * sizeof(GLdouble),
		(GLvoid *)mesh->vert, GL_STATIC_DRAW); //thing to save in this data
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, meshGL->buffers[1]); //bind them to a buffer 1
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh->triNum * 3 * sizeof(GLuint),
		(GLvoid *)mesh->tri, GL_STATIC_DRAW); //performance hints: GL_STATIC_DRAW
}
