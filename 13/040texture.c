...
void texSample(texTexture *tex, double s, double t) {
	...
	if (tex->filtering == texQUADRATIC) {
         ...
         * uFrac*vFrac(texGetTexel(ceil(u),ceil(v)))            [1]
         * + uFrac*(1-vFrac)(texGetTexel(ceil(u),floor(v)))     [2]
         * + (1-uFrac)*vFrac(texGetTexel(floor(u),ceil(v)))     [3]
         * + (1-uFrac)*(1-vFrac)(texGetTexel(floor(u),floor(v)))[4]
         */
        double Ufrac = u - floor(u);//ufrac
        double Vfrac = v - floor(v);//vfrac
        
        //the scalar part of each addition
        double uv = Ufrac*Vfrac;
        double uv1 = Ufrac*(1-Vfrac);
        double u1v = (1-Ufrac)*Vfrac;
        double u1v1 = (1-Ufrac)*(1-Vfrac);
        
        //to save the total rgb values
        double total[tex->texelDim];
        
        for(i=0; i<tex->texelDim; i++)
        {
            //zeroing everyting out to make sure any left over values aren't interfering
            tex->sample[i] = 0;
            tex->aux[i] = 0;
            total[i] = 0;
        }
        ...
        vecScale(tex->texelDim, uv, tex->aux, tex->sample);
        for(i=0; i<tex->texelDim; i++)//set aux to zero to use it again
            tex->aux[i] = 0;
        vecAdd(tex->texelDim, tex->sample, total, tex->aux);
        vecCopy(tex->texelDim, tex->aux, total);
        for(i=0; i<tex->texelDim; i++)
        {
            tex->sample[i] = 0;
            tex->aux[i] = 0;
        ...
        vecScale(tex->texelDim, uv1, tex->aux, tex->sample);
        for(i=0; i<tex->texelDim; i++)
            tex->aux[i] = 0;
        vecAdd(tex->texelDim, tex->sample, total, tex->aux);
        vecCopy(tex->texelDim, tex->aux, total);
        ...
        vecScale(tex->texelDim, u1v, tex->aux, tex->sample);
        for(i=0; i<tex->texelDim; i++)
            tex->aux[i] = 0;
        vecAdd(tex->texelDim, tex->sample, total, tex->aux);
        vecCopy(tex->texelDim, tex->aux, total);
        ...
        vecScale(tex->texelDim, u1v1, tex->aux, tex->sample);
        for(i=0; i<tex->texelDim; i++)
            tex->aux[i] = 0;
        vecAdd(tex->texelDim, tex->sample, total, tex->aux);
        vecCopy(tex->texelDim, tex->aux, total);
