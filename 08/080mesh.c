...
double varying[renVARYDIMBOUND * renVERTNUMBOUND];
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], 
		texTexture *tex[]) {
	/* meshRender: 
	 * Fill in its implementation, using other functions such as meshGetTrianglePointer, 
	 * meshGetVertexPointer, and triRender. Don't overlook the part of the 
	 * specification about attrDim.
	 */

	if(mesh->attrDim != ren->attrDim)
		fprintf(stderr,"Error! mesh->attrDim != ren->attrDim\n");
	...
	for (i = 0; i < mesh->triNum; i++)
	{
		...
		//call triRender on each triangle
	}
}
